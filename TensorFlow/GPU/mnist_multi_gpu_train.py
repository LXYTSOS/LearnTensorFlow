# -*- coding: utf-8 -*-
"""
Created on Mon Jul 31 11:04:24 2017
...
DATA_PATH = "../tfrecordfiles/output.tfrecords" 
...
# 定义损失函数。计算在一个命名空间下的总损失，之所以要给定命名空间是因为不同GPU上
#计算得出的正则化损失都会加入名为loss的集合，如果不通过命名空间就会将不同GPU上
#正则化损失都加进来
         ...
         for i in range(N_GPU):
            ...
            with tf.device('/gpu:%d' % i):
                with tf.name_scope('GPU_%d' % i) as scope:
                    ...
                    #在第一次声明变量后，将控制变量重用的参数设为True，这样可以让
                    #不同GPU更新同一组参数
         ...
         with tf.Session(config=tf.ConfigProto(
            ...
            for step in range(TRAINING_STEPS):
                ...
                if step != 0 and step % 10 == 0:
                    # 计算使用过的训练数据个数。因为在每一次运行训练操作时，每一个
                    #GPU都会使用一个batch的数据，所以总共用到的训练数据个数为
                    #batch*GPU个数
                    num_examples_per_step = BATCH_SIZE * N_GPU
                    #duration为运行当前训练过程使用的时间，所以平均每秒可以处理的
                    #训练数据个数为num_examples_per_step / duration
                    examples_per_sec = num_examples_per_step / duration
                    #因为在每一个训练过程中，每个GPU都会使用一个batch，所以在单个
                    #batch上的训练时间为duartion / GPU个数
