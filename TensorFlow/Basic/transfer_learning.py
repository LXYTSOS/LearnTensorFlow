# -*- coding: utf-8 -*-
"""
Created on Wed Jul 12 15:54:17 2017
...
#Inception-v3模型中代表瓶颈层结果张量的名称，在谷歌提供的Inception-v3模型中，
#这个张量的名称就是'pool_3/_reshape:0'。在训练模型时，可以通过tensor.name
#来获取张量名称
...
#下载的谷歌训练好的Inception-v3模型目录
MODEL_DIR = '../model/inception'

#下载的谷歌训练好的Inception-v3模型名称
MODEL_FILE = 'tensorflow_inception_graph.pb'

#因为一个训练数据会被使用多次，所以可以将原始图像通过Inception-v3模型计算得到
#的特征向量保存在文件中，避免重复计算。下面的变量定义了这些文件的存放地址。
CACHE_DIR = '../tmp/bottleneck'

#图片数据文件夹。在这个文件夹中每一个子文件夹代表了一个需要区分的类别，
#每个子文件夹中存放了对应的图片
INPUT_DATA = '../flowerdata/flower_photos'
...
#testing_percentage和validation_percentage参数指定了测试数据和验证数据集的大小。
def create_image_lists(testing_percentageg, validation_percentage):
    #得到的所有图片都存在result这个字典里，这个字典的key为类别的名称，value
    #也是一个字典，字典里存储了所有的图片名称。
    result = {}
    #获取当前目录下所有子目录
    sub_dirs = [x[0] for x in os.walk(INPUT_DATA)]
    #得到的第一个目录(flower_photos)是当前目录，不需要考虑
    is_root_dir = True
    for sub_dir in sub_dirs:
        ...
        for file_name in file_list:
            ...
            #随机将数据分到训练数据集、测试数据集和验证集
            chance = np.random.randint(100)
            if chance < validation_percentage:
                validation_images.append(base_name)
            elif chance < (testing_percentageg + validation_percentage):
...
#image_dir参数给出了根目录。存放图片数据根目录和图片特征向量的根目录不同
#label_name参数给出了类别名称
#index参数给出了需要获取的图片的编号
#category指定了需要获取的图片是在训练数据集、测试数据集还是验证数据集
...
def run_bottleneck_on_image(sess, image_data, image_data_tensor,
                            bottleneck_tensor):
    #这个过程其实就是将当前图片作为输入计算瓶颈张量的值，这个值就是这个图片的
    #特征向量
    bottleneck_values = sess.run(bottleneck_tensor,
                                 {image_data_tensor: image_data})
    #经过卷积神经网络处理的结果是一个四维数组，需要将这个结果压缩成
    #一个特征向量（一维数组）
    bottleneck_values = np.squeeze(bottleneck_values)
    return bottleneck_values

#这个函数获取一张图片经过Inception-v3模型处理之后的特征向量，这个函数会先
#试图寻找已经计算且保存下来的特征向量，如果找不到则优先计算这个特征向量，
#然后保存到文件
def get_or_create_bottleneck(sess, image_lists, label_name, index,
    ...
    bottleneck_path = get_bottleneck_path(image_lists, label_name,
                                          index, category)
    #如果这个特征向量不存在，则通过Inception-v3模型来计算特征向量，并
    #将计算结果存入文件
...
#这个函数获取全部的测试数据，在最终测试的时候需要在所有测试数据上计算正确率
...
def main(_):
    ...
    #读取已经训练好的Inception-v3模型，谷歌训练好的模型保存在了GraphDefProtocolBuffer
    #中，里面保存了每一个节点的取值的计算方法以及变量的取值。
    ...
    #定义新的神经网络输入，这个输入就是新的图片经过Inception-v3模型前向传播
    #到达瓶颈层时的节点取值，可以将这个过程类似的理解成一种特征提取。
    ...
    cross_entorpy_mean = tf.reduce_mean(cross_entropy)
    train_step = tf.train.GradientDescentOptimizer(LEARNING_RATE).minimize(cross_entorpy_mean)
        ...
        for i in range(STEPS):
            ...
            if i % 100 == 0 or i + 1 == STEPS:
                ...
                print('Step %d: Validation accuracy on random sampled %d '
                   'example = %.1f%%' %(i, BATCH, validation_accuracy*100))
